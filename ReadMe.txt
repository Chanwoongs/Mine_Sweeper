...
현재 나는 오류 : 많은 마우스 클릭 후 게임이 종료 될 때 ~Map()에서 delete 에러 발생
...
오류 발견 
setNearBlocks 함수에서 선택한 블럭 주위 9개를 모두 처리하는 과정에서 메모리 초과 발생
오류 수정
1. 하나 씩 Block을 여는 함수로 변경 후 오류가 안남 확인
2. nearMines의 값을 0이 아닌 -1로 초기화를 하는 예외 처리 후 메모리 초과 안남 확인	  
